...
| 🔗 対象者（雇用条件書・複数可） | `koyou_details` | サブテーブル | ○ | 1行=1申請人。反映先の雇用条件書レコード（§10：事前作成運用）。列は下表 |
...
**サブテーブル `koyou_details`（対象者・複数人）の列**

| フィールド名 | フィールドコード | 種類 | 用途・備考 |
|------------|----------------|------|-----------|
| 雇用条件書 | `koyou_ref` | 数値（ルックアップ→app55） | 反映先の app55 レコード番号 |
| 人材ID（自動） | `koyou_hrid` | 数値 | app55 の HRID を自動コピー。FunBase メンバー解決に使う |
| 申請人（自動） | `koyou_applicant_disp` | 文字列(1行) | 申請人氏名の自動コピー（表示用） |
| 反映 | `koyou_sync_status` | ドロップダウン | 未反映/反映済/エラー。**人ごと**の反映結果（転記後に書き戻す） |

> ⚠️ 雇用条件書の反映ステータスは**この行単位の `koyou_sync_status`** が正。
> レコード直下の `sync_koyou_status`（1案件=1申請人だった頃の名残）は kintone 側から廃止済みなので書かないこと。
> サブテーブルの更新は「送信した行での置換」なので、**更新しない行も含め全行を行ID付きで送る**（送らない行は消える）。
...
- 反映ステータスは `sync_company_status`（法人・レコード直下）と `koyou_sync_status`（雇用条件書・`koyou_details` 行単位）。いずれも既定「未反映」
- フォームレイアウト＝4セクション（基本情報／紐付け／書類・Drive／反映管理）に整理済
- **1案件=複数申請人**（`koyou_details` サブテーブル・2026-08-04 に移行済）。`koyou_ref` と反映ステータスはテーブル内へ移動し、レコード直下の `sync_koyou_status` は廃止
