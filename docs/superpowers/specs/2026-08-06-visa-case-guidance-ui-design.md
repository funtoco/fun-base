...
内容は逐語転記だが、次の4か所だけ読点・数字幅・括弧を整えている（意味は変えていない）。実装 `lib/portal/guidance/documents.ts` はこの表記に一致する。

- 法人 No.2 備考: `…役員の場合は、書類作成責任者の…`（読点を追加）
- 法人 No.4 書類名: `下記3つ`（PDF は全角 `３`）
- 法人 No.5 書類名: `…領収証書（①②のいずれか）`（PDF は括弧なしで併記）
- 法人 No.6 備考: `社会福祉法人の場合は不要です。`（`は` を追加）
...
|---|---|---|---|---|
| 1 | 履歴事項全部証明書 | 原本 | 法務局 | — |
| 2 | 住民票（本籍地記載あり / マイナンバー記載無） | 原本 | 市区町村役場 | 業務執行に関与する役員 = 特定技能受け入れに関わる役員です。<br>代表者の住民票をご用意ください。ただし、書類作成責任者が代表者ではない役員の場合は、書類作成責任者の住民票をご用意ください。 |
| 3 | 労働保険料等納付証明書 | 原本 | 労働局 | （資料に「詳細はこちら」リンクあり・URL未取得） |
| 4 | 税務署発行の納税証明書（その３）<br>税目は下記3つ全てが必要です。①源泉所得税及び復興特別所得税 ②法人税 ③消費税及び地方消費税 | 原本 | 税務署 / 国税庁 | 市税事業所ではない点にご注意ください。<br>（資料に「詳細はこちら」リンクあり・URL未取得） |
| 5 | ①社会保険料納入状況照会回答票 ②健康保険 / 厚生年金保険料領収証書（①②のいずれか） | ①原本 ②写し | 年金機構 | 申請する月の前々月までの24か月分が必要です。<br>（資料に「詳細はこちら」リンクあり・URL未取得） |
| 6 | 法人住民税納税証明書 | 原本 | 市税事業所 | 直近1年度分が必要です。<br>社会福祉法人の場合は不要です。 |
| 7 | [外食] 保健所長の営業許可証 / [宿泊] 旅館業許可証 | 写し | — | 就業する事業所全ての営業許可書が必要です。<br>有効期限内のものをご用意ください。 |
| 8 | 特定技能協議会加入証明書 | 写し | — | [介護分野の場合] 配属先事業所がご登録されているかご確認ください。 |
...
| プリセット | x | W | H | 出力実寸 |
|---|---|---|---|---|
| `half-left` | 0 | 877 | 1071 | 982 × 1200 |
| `half-right` | 877 | 877 | 1071 | 982 × 1200 |
| `full` | 0 | 1754 | 1071 | 1200 × 733 |

**上端 `y` は PDF ごとに異なる。** 2本の資料は別の版下を使っており、法人向けのほうがスライドの中身が約 55px 上にある。
スライド見出し「取得書類一覧サンプル」を落として書類名と No. を残す位置は、個人事業主向けが `y = 170`、法人向けが `y = 137`。
実測した ink 行（150dpi）は次のとおり。

| 資料 | スライド見出し | 書類名 + No. | 書類画像 |
|---|---|---|---|
| 個人事業主向け | 125–150 | 196–219 | 253〜 |
| 法人向け | 98–135 | 140–166 | 196〜 |

法人向けは見出し下端 136 と書類名上端 140 の間しか猶予がないため、この値はタイトである。
当初 `y = 170` を全体に適用したところ、法人向け8枚すべてで書類名が欠けた。

**リサイズは `sips -Z` ではなく `sips -z <height> <width>` を使う。** `-Z` は長辺を合わせる際に切り捨てるため、
full プリセットが 1071 × 1200/1754 = 732.73 → 732 となり、`samples.ts` の `FULL = 733` と1px ずれる。
プリセットごとに目標寸法を明示して等倍指定することで、表の実寸どおりに出力される。
...
これらは元資料の時点でマスキング済みで、既にメール配布している内容のため `public/`（未認証アクセス可）に置く。

ただし `sp-tax-certificate-3.jpg` と `corp-tax-certificate-3.jpg` の下部にある **QR コードは黒塗りされていない**。
日本の納税証明書の QR は証明内容を符号化しているのが通例で、原典 PDF の時点でのマスキング漏れである。
黒塗り・当該2枚の除外・認証必須化のいずれも検討したうえで、**利用者の判断でそのまま公開する**こととした（2026-08-06）。
元資料の配布範囲が既に広く、追加のリスクが小さいという判断による。方針を変える場合は、
切り出しスクリプトに塗りつぶし工程を足すか、API ルート経由の配信に変更する。
